...
rooms.post("/join", async (c) => {
  ...
  // The real session starts now that both users are present: regenerate the pool
  // (and clear any pre-join swipes) so BOTH profiles' preferences count — otherwise
  // a deck generated by user_a before user_b joined would ignore user_b's prefs.
